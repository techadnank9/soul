...
/**
 * A week of demo entries for one account, written from nothing.
 *
 * Used by the seed script for the test student and by the demo skip in the
 * app, which makes a fresh account and fills it so that whoever presses skip
 * lands on a home with something in it.
 */
export async function seedDemoWeek(scope: {
  studentId: string
  schoolId: string
  districtId: string
}): Promise<number> {
  const student = { id: scope.studentId }
  const school = { id: scope.schoolId }
  const district = { id: scope.districtId }
